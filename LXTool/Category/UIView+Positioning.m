...
-(CGPoint)boundCenter{
    return (CGPoint){self.boundCenterX, self.boundCenterY};
}
...
-(void)centerToParent{
    if(self.superview){
        switch ((NSUInteger)[UIApplication sharedApplication].statusBarOrientation){
...
- (BOOL)isContainedPoint:(CGPoint)po{
    return (po.x < (self.x+self.width))&&(po.y < (self.y+self.height));
}
